...
class SpineC2M(object):
    def __init__(self, flags):
        self.flags = flags
        self.name = 'DC2Anet_db'
        self.image_size = (256, 256, 1)
        self.num_tests = 129

        # tfrecord path
        self.train_tfpath = os.path.abspath("DC2Anet_db/tfrecords/train.tfrecords")
        self.test_tfpath = os.path.abspath("DC2Anet_db/tfrecords/test.tfrecords")
    ...
    def __call__(self, is_train='True'):
        if is_train:
            if not os.path.abspath("DC2Anet_db/tfrecords/train.tfrecords"):
                sys.exit(' [!] Train tfrecord file is not found...')
            return self.train_tfpath
        else:
            if not os.path.abspath("DC2Anet_db/tfrecords/test.tfrecords"):
